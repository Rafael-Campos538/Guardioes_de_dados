class Cenainicial extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.spritesheet('menina1', 'assets/personsasa/menina1.png', { frameWidth: 64, frameHeight: 64 });

        this.load.spritesheet('passarinho', 'assets/imagens/passarinho.png', { frameWidth: 384, frameHeight: 384 });
    ...
    create() {
        ...
        const background = this.add.image(0, 0, "Cena1").setOrigin(0, 0).setDisplaySize(this.cameras.main.width * 1.5, this.cameras.main.height * 1.5);
        ...
        //passarinho
        this.passarinho = this.add.sprite(this.xi, this.yi, 'passarinho').setScale(0.2);

        this.anims.create({
            key: 'fly',
            frames: this.anims.generateFrameNumbers('passarinho', { start: 0, end: 8 }), //animaçao do passarinho
            frameRate: 10,
            repeat: -1
        });
    ...
    update(delta) {
        ...
        if (this.passarinho && this.t <= this.t_total) {
            ...
            console.log(`Tempo: ${this.t.toFixed(2)}s - X: ${this.x.toFixed(2)}m - Velocidade X: ${this.vx.toFixed(2)}m/s`);

            // Atualizando posição no eixo Y (MUV)
            this.vy = this.ay * this.t;
            this.y = this.yi + (this.vy * this.t) / 2;
            this.passarinho.y = this.y;
            console.log(`Tempo: ${this.t.toFixed(2)}s - Y: ${this.y.toFixed(2)}m - Velocidade Y: ${this.vy.toFixed(2)}m/s - Aceleração Y: ${this.ay.toFixed(2)}m/s²`);
        }
    }
}
